...
//模板style
let templateStyle: HTMLStyleElement | undefined;
 ...
 * @param appName 应用名称
 */
export default function scopedCss(styleElement: HTMLStyleElement, appName: string): void {
  //前缀
 ...
 * @returns
 */
export function scopedRule(rules: Array<CSSRule>, prefix: string): string {
  let result = "";
  //遍历rules，处理每一条规则
  for (const rule of rules) {
    ...
    switch (rule.type) {
      case 1:
        result += scopedStyleRule(<CSSStyleRule>rule, prefix);
        break;
      case 4:
        result += scopedPackRule(<CSSMediaRule>rule, prefix, "media");
        break;
      case 12:
        result += scopedPackRule(<CSSSupportsRule>rule, prefix, "supports");
        break;
      default:
        result += rule.cssText;
        break;
 ...
 * @returns
 */
export function scopedPackRule(
  rule: CSSMediaRule | CSSSupportsRule,
  prefix: string,
  packName: string
): string {
  //递归执行scopedRule，处理media 和 supports内部规则
  const result: string = scopedRule(Array.from(rule.cssRules), prefix);
 ...
 */
export function scopedStyleRule(rule: CSSStyleRule, prefix: string): string {
  // 获取CSS规则对象的选择和内容
